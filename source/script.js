function run(jason){
	var jason = preprocess_data(jason);
	[
		Title(),
		ChartView(jason),
		OverView(jason),
...
function popular_set(list, comparator){
	let popular = [];
	let count = 0;
...
function prettyList(list, wrapStart="", wrapEnd=""){
	if(list.length == 0)
		return '';
	else if(list.length == 1)
		return wrapStart+list[0]+wrapEnd;
	else
		return wrapStart+list.slice(0,-1).join(wrapEnd+', '+wrapStart)+wrapEnd+' and '+wrapStart+list[list.length-1] + wrapEnd;
}

function preprocess_data(jason){
	...
	Object.keys(jason.movies).forEach(function(key){
		...
		movie.directors.forEach(function(d){ d.movies.push(jason.movies[key]) });
		movie.countries = jason.movies[key].country.map(function(c){ return jason.countries[c] });
		movie.countries.forEach(function(c){ c.movies.push(jason.movies[key]) });
		movie.bagged = 0;
		movie.mentioned = 0;
		movie.visitors = [];
		movie.length = 0;
...
function ChartView(jason){
	...
	all.append("rect").attr("class", "bagged")
		...
		.attr("height", function(d){ return d.bagged*bin_height });
	all.append("rect").attr("class", "mentioned")
		.attr("width", bin_width)
		.attr("height", function(d){ return d.mentioned*bin_height });
...
function OverView(jason){
	...
	let movies = popular_set(jason.movie_list, function(m){ return m.length });
	let directors = popular_set(jason.directors, function(d){ return d.movies.length });
	let countries = popular_set(jason.countries, function(c){ return c.movies.length });
	let i18n = {
		...
		movies_list: prettyList(movies.popular.map(function(m){ return m.title }),'<cite class="interactive">','</cite>'),
		...
		directors_list: prettyList(directors.popular.map(function(d){ return d.name }), '<span class="interactive">', '</span>'),

		country: (countries.popular.length > 1) ? 'countries' : 'country',
		countries_be: (countries.popular.length > 1) ? 'are' : 'is',
		countries_list: prettyList(countries.popular.map(function(c){ return c.name }), '<span class="interactive">', '</span>'),
	...
	element.appendChild(document.createElement('p')).innerHTML = 'Some general stats -- there have been <span class="interactive">'+jason.stats.visitsCount+'</span> visits to the Criterion closet, from which <span class="interactive">'+jason.stats.moviesCount+'</span> unique films by <span class="interactive">'+jason.stats.directorsCount+'</span> unique directors in <span class="interactive">'+jason.stats.countriesCount+'</span> unique countries have been picked up.';
...
function VisitListView(jason){
	return ListView(
		...
		function(it){
			let movieList=prettyList(it.all_movies.map(function(m){ return '<cite class="'+(it.spines_bagged.indexOf(m.spine_number)==-1?'mentioned':'bagged')+'">'+m.title+'</cite>'; }))+'.';
			return '<a href="https://youtu.be/'+it.url+'">Video link</a>.<br/>'+movieList + (it.notes.length > 0? '<hr/>'+it.notes : '');
		},
		{
			'date': function(a,b){
			...
			'alphabetically': function(a,b){
				return (a.visitor > b.visitor) ? 1 : ((b.visitor > a.visitor) ? -1 : 0);
			},
			'greediness': function(a,b){
				if (b.all_movies.length > a.all_movies.length)
					return 1;
				if (a.all_movies.length > b.all_movies.length)
					return -1;
				return (a.date > b.date) ? 1 : ((b.date > a.date) ? -1 : 0);
			}
		}
...
function MovieListView(jason){
	return ListView(
		...
		function(it){
			return 'Directed by '+prettyList(it.directors.map(function(d){return d.name}))+'. '+it.year+'. <a href="https://criterion.com/films/'+it.url+'">Criterion link</a>.<br/>'+ prettyList(it.visitors.map(function(v){ return '<cite class="'+(v.spines_bagged.indexOf(it.spine_number)==-1?'mentioned':'bagged')+'">'+v.visitor+'</cite>' }))+'.'
		},
		{
			'spine #': function(a,b){
				return (a.spine_number > b.spine_number) ? 1 : ((b.spine_number > a.spine_number) ? -1 : 0);
			},
			'popularity': function(a,b){
				if(b.visitors.length > a.visitors.length)
					return 1;
